...
2. Re-run the same raw INSERT statements used in scenario 2
   (`02_out_of_order_lab_result.sql`) directly against the Iceberg CDC table
...
- Reprocessing already-applied events converges to the same state as before
  ...
  regression to a stale value — this is exactly what scenario 2 verifies,
