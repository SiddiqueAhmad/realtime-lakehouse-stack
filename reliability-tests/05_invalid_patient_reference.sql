...
-- lake. We reproduce that by inserting directly into
-- warehouse.raw_cdc.diagnoses — the append-only landing table Debezium's
-- JDBC sink writes into — referencing a patient_id that doesn't exist.
--
-- Run: psql -h localhost -p 5433 -U testuser -d warehouse -f 05_invalid_patient_reference.sql
--
...
VALUES (9002, 1, 999999, 'Z00.00', 'Encounter for general adult medical examination',
        ...
        CAST(extract(epoch from TIMESTAMP '2026-03-01 09:00:00') * 1e9 AS BIGINT), 900000003, 'ehr');
