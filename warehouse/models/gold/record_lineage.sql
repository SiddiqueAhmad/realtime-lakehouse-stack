...
-- answering "why is this record in the state it's in":
--
--   Patient record
--        |
...
-- analyst or an AI agent can investigate "why did this record end up
-- quarantined" without ever touching a clinical value, and escalate to a
-- clinical_user/data_engineer (who can resolve record_token back to the
-- underlying row in bronze/silver under their own tier's access) only if
-- the investigation actually needs the PHI itself.
