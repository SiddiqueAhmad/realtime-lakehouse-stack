...
--
-- lag() over the same (record_token, logged_at) ordering the model itself
-- uses to find "the last logged decision" is the correct way to express
-- "consecutive" here - not a self-join on adjacent row numbers, which
-- would need its own tie-breaking logic that could drift from the model's.
