#!/usr/bin/env bash
# Tests ONLY the schema migration in a newly created scratch database.
# The normal governance database and all object-storage files are untouched.
set -euo pipefail
...
pg() {
  "${compose[@]}" exec -T postgres \
    psql -X -v ON_ERROR_STOP=1 -U governance -d "$test_db" "$@"
}
cleanup() {
  local rc=$?
  trap - EXIT
  if [[ "$created" == 1 ]]; then
    if ! "${compose[@]}" exec -T postgres \
      psql -X -v ON_ERROR_STOP=1 -U governance -d postgres \
      -c "DROP DATABASE \"$test_db\";" >/dev/null; then
      echo "Could not remove scratch database $test_db" >&2
      rc=1
    fi
  fi
  exit "$rc"
...
# Reproduce the pre-JSONB schema. Dummy policy payloads deliberately prove
# the migration preserves source text; Cedar behavior is tested separately.
pg >/dev/null <<'SQL'
CREATE SCHEMA governance;
CREATE TABLE governance.principals (
    principal_key text PRIMARY KEY, role text NOT NULL,
...
BEGIN
    ...
    END IF;
    IF (SELECT count(*) FROM governance.principals) <> 3 THEN
        RAISE EXCEPTION 'principal rows changed';
    END IF;
    IF EXISTS ((SELECT * FROM governance.policies EXCEPT SELECT * FROM public.saved_policies)
               UNION ALL
               (SELECT * FROM public.saved_policies EXCEPT SELECT * FROM governance.policies)) THEN
        RAISE EXCEPTION 'policy source, enabled flag, or timestamp changed';
    END IF;
    IF (SELECT count(*) FROM governance.policy_bindings) <> 5 THEN
        RAISE EXCEPTION 'expected five bindings';
    END IF;
    ...
    END IF;
    IF (SELECT value FROM public.unrelated_data) IS DISTINCT FROM 42 THEN
...
SQL
echo 'PASS: legacy upgrade preserves principals, region, policies, and unrelated data'
...
BEGIN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM governance.policy_bindings WHERE binding_id='bind_region_analyst') THEN
        RAISE EXCEPTION 'rerun resurrected deleted binding';
    END IF;
    IF (SELECT enabled FROM governance.policy_bindings WHERE binding_id='bind_legal_hold_global') IS DISTINCT FROM false THEN
        RAISE EXCEPTION 'rerun re-enabled a disabled binding';
    END IF;
END;
$$;
SQL
echo 'PASS: rerun is a no-op, including after binding deletion/disablement'

# This DROP is inside the unique scratch database, never the live demo DB.
...
BEGIN
    IF (SELECT attributes FROM governance.principals WHERE principal_key='analyst') IS DISTINCT FROM '{"region":"us-west","custom":"keep"}'::jsonb THEN
        RAISE EXCEPTION 'migration changed an already-dynamic principal';
    END IF;
    IF EXISTS (SELECT 1 FROM governance.policy_bindings WHERE binding_id='bind_region_analyst') THEN
        RAISE EXCEPTION 'migration restored a binding in an already-dynamic schema';
    END IF;
END;
$$;
SQL
echo 'PASS: existing dynamic schema is preserved'
